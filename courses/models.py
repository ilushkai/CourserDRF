...
class Payments(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, verbose_name='Пользователь', related_name='user')
    date_of_payment = models.DateField(auto_now_add=True, verbose_name='Дата покупки')
    paid_course = models.ForeignKey(Course, on_delete=models.PROTECT, verbose_name='оплаченный курс', related_name='paid_course', **NULLABLE)
    paid_lesson = models.ForeignKey(Lesson, on_delete=models.PROTECT, verbose_name='оплаченный Урок', related_name='paid_lesson', **NULLABLE)
